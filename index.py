...
app.config['MYSQL_HOST'] = 'localhost'  # Endereço.venv do servidor MySQL
...
def add_task():
    ...
    if request.method == 'POST':  # Verifica se o método da requisição é POST
        ...
        conn, cur = get_cursor()  # Cria um cursor
...
def delete_task(id):
    ...
    conn, cur = get_cursor()  # Cria um cursor
    cur.execute(
        "UPDATE tasks SET task_status = 'del' WHERE task_id=%s",
        (id,)
    )  # Marca a tarefa como deletada
    conn.commit()  # Confirma a transação
    cur.close()  # Fecha o cursor
    conn.close()
    return redirect(url_for('home'))  # Redireciona para a página inicial
